...
const Manage_User = () => {
    ...
    const handleDeleteUser = (idUser) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                if (response?.success)
                    Swal.fire('Đã xóa!', 'Người dùng đã được xóa thành công.', 'success').then((confirm) => {
                        if (confirm.isConfirmed) {
                            setData((prev) => prev.filter((element) => element._id !== idUser));
                        }
                    });
    ...
    return (
        <div className="bg-white shadow-md p-8 rounded-md my-4">
                                ...
                                <tbody>
                                    {data?.map((user, index) => (
                                        <tr key={user._id} className="text-center text-lg">
                                            ...
                                            <td className=" h-[80px] py-2 border px-3 text-center">
                                                <div className=" flex justify-center items-center gap-4">
                                                    ...
                                                    <span className="uppercase text-sm font-semibold">
                                                        {user.isBlocked ? 'block' : 'actived'}
                                                    </span>
                                                </div>
                                            </td>
                                            <td className=" h-[80px] border py-2 px-3 ">{moment(user.createdAt).format('DD/MM/YYYY')}</td>
                                            <td
                                                className={`h-[80px] border py-2 px-3 flex gap-5 items-center justify-center text-main capitalize`}
                                            >
                                                <div className=" flex justify-center items-center gap-4">
                                                    <Link to={`/admin/manage-user/${user._id}`}>
                                                        <span className="flex items-center justify-center h-[80px] hover:opacity-80 cursor-pointer hover:underline">
                                                            edit
                                                        </span>
                                                    </Link>
                                                    <span
                                                        onClick={() => handleDeleteUser(user._id)}
                                                        className="flex items-center justify-center h-[80px] hover:opacity-80 cursor-pointer hover:underline"
                                                    >
                                                        delete
                                                    </span>
                                                </div>
