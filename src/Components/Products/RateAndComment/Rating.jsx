...
const Rating = ({ ratings, totalRatings, title, pid }) => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { isLogin } = useSelector((state) => state.user);
    const handleVoteClick = () => {
        if (isLogin) {
            dispatch(showModal({ isShowModal: true, modalChildren: <VoteOption nameProduct={title} pid={pid} /> }));
        } else {
            Swal.fire({
                text: 'Login to vote',
            ...
            <div className="flex w-full">
                ...
                <div className="flex-6 pl-5 flex flex-col gap-3 border rounded-r-[20px] p-4">
                    {Array.from(Array(5).keys())
                        .reverse()
                        .map((item) => (
                            <VoteBar rating={item + 1} key={item} countRating={0} totalRatings={totalRatings || 1} />
                        ))}
                </div>
            </div>
